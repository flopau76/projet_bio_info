...
def read_fasta2dict(filename):
  """Read a FASTA file and return a list of keys and a dictionary of sequences"""
...
def compare_seq(refseq, refkeylist, testseq,testkeylist, verbose=False):
# Number of sequences
  nseqs = len(testkeylist)
  if verbose: print('nseqs = %d' % nseqs)
  ...
  if verbose: print('ntestcols = %d' % ntestcols)
  ...
  if verbose: print('nrefcols = %d' % nrefcols)
  # verify that all sequences have the same number of columns
...
if __name__ == "__main__":     
  ...
  refkeylist, refseq = read_fasta2dict(reffile)
  testkeylist, testseq = read_fasta2dict(testfile)
